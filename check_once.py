...
def extract_area(text: str) -> str:
    match = re.search(r"(\d+(?:[.,]\d+)?)\s*(?:m²|m2|მ²)", text, re.IGNORECASE)
...
def extract_location(text: str) -> str:
    ...
    if "saburtalo" in lowered or "საბურთალო" in lowered or "сабуртало" in lowered:
...
def listing_id_from_url(site: str, url: str) -> str | None:
    if site == "MyHome.ge":
        ...
        match = re.search(r"/real-estate/(?!l/)[^?#]*-(\d+)(?:[/?#]|$)", url)
...
async def scrape_all_sources() -> tuple[list[dict[str, Any]], list[str]]:
    ...
    async with async_playwright() as playwright:
        ...
        for source in SOURCES:
            ...
            try:
                ...
                for candidate in await collect_page_links(page):
                    ...
                    if rooms is None or rooms not in source["rooms"]:
                        continue
                ...
                errors.append(f"{source['key']}: timeout")
...
def format_listing(item: dict[str, Any]) -> str:
    deal = "Аренда" if item["deal"] == "rent" else "Продажа"
    safe_url = html.escape(item["url"], quote=True)
    return (
        "🏠 <b>НОВОЕ ОБЪЯВЛЕНИЕ</b>\n\n"
        f"🌐 <b>Сайт:</b> {html.escape(item['site'])}\n"
        f"🔑 <b>Тип:</b> {deal}\n"
        f"📍 <b>Район:</b> {html.escape(item['location'])}\n"
        f"🚪 <b>Комнат:</b> {item['rooms']}\n"
        f"📐 <b>Площадь:</b> {html.escape(item['area'])}\n"
        f"💰 <b>Цена:</b> {html.escape(item['price'])}\n\n"
...
async def main() -> int:
    ...
    for item in listings:
        ...
        if listing_id not in seen:
            if initialized and int(listing_id) > int(state["max_ids"][key]):
    ...
    if not initialized:
        ...
        send_telegram(
            "✅ Облачный мониторинг включён. Существующие объявления сохранены; "
            "дальше уведомления будут приходить только о новых объявлениях."
        )
    else:
        new_items.sort(key=lambda item: int(item["listing_id"]))
        for item in new_items:
            send_telegram(format_listing(item))

    # A weekly harmless state change keeps GitHub's public scheduled workflow active.
    state["heartbeat_week"] = datetime.now(timezone.utc).strftime("%G-W%V")
    save_state(state)
    print(f"Found {len(listings)} listings; sent {len(new_items)} notifications")
    return 0
