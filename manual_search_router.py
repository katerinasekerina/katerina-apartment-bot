...
- Apartments only
- 1, 2 or 3 rooms
- Maximum result count is controlled by MANUAL_LIMIT
- Supports current MyHome /en/pr/<ID>/ listing URLs
"""
...
# ---------------------------------------------------------------------------
# MANUAL-SEARCH COMPATIBILITY FIXES
# ---------------------------------------------------------------------------
#
# IMPORTANT:
# These patches are loaded only when manual_search_router.py runs.
# The normal automatic production workflow still runs check_once.py directly,
# so its production monitoring logic remains untouched.
#
...
def manual_listing_id_from_url(site: str, url: str):
    """Recognize current MyHome search-card URLs during manual searches."""
    ...
    if site == "MyHome.ge":
        # Current MyHome search cards commonly use:
        # /en/pr/25174661/2-room-apartment-for-rent-in-vashlijvari/
        #
        # Georgian or another locale may use the same structure with
        # a different two-letter locale.
        match = re.search(
            r"/(?:[a-z]{2}/)?pr/(\d{6,10})(?:/|$)",
            path,
            re.IGNORECASE,
    ...
    # Keep all previously supported URL formats as a fallback.
...
def manual_is_target_location(
    ...
    """Apply manual-search-only safety checks before district matching."""
    ...
    if source and source.get("manual_search"):
        # Manual mode is RENT ONLY.
        #
        # This is an additional safety layer in case SS.ge or MyHome
        # unexpectedly exposes a sale result on a rental results page.
        sale_markers = (
            "for sale",
            "/for-sale",
            "flat-for-sale",
    ...
    # District verification remains exactly the one already used by
    # the stable check_once.py logic.
...
def manual_sources_for_district(district_key: str):
    ...
    # SS.ge:
    # For-Rent endpoint
    # individualEntityOnly = physical/private owner filter
    ...
    return [
        {
            **shared,
            ...
            "site": "MyHome.ge",
            "url": (
                "https://www.myhome.ge/en/real-estate/"
                "rent/apartment/"
...
def main() -> int:
    # Patch only MANUAL search configuration.
    #
    # Automatic production:
    # check-apartments.yml -> check_once.py
    #
